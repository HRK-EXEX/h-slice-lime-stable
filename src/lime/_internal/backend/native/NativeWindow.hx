...
class NativeWindow
{
	...
	// finally i can edit here for officially lmfao
	var imageBuffer:ImageBuffer = null;
	var readGL:WebGLRenderContext = null;
	var readArrayData:UInt8Array = null;
	var swapBytes:Bytes = null;
	var readBuffer:ArrayBuffer = null;
	public function readPixels(rect:Rectangle):Image
	{
		...
		switch (parent.context.type)
		{
			case OPENGL, OPENGLES, WEBGL:
				...
				if (readArrayData == null) readArrayData = new UInt8Array(width * height * 4);

				readGL.readPixels(x, y, width, height, readGL.RGBA, readGL.UNSIGNED_BYTE, readArrayData);
				...
				swapBytes = Bytes.alloc(rowLength);
				readBuffer = readArrayData.buffer;
				var rows = Std.int(height / 2);

				while (rows-- > 0)
				{
					swapBytes.blit(0, readBuffer, destPosition, rowLength);
					readBuffer.blit(destPosition, readBuffer, srcPosition, rowLength);
					readBuffer.blit(srcPosition, swapBytes, 0, rowLength);
				...
				imageBuffer = new ImageBuffer(readArrayData, width, height, 32, RGBA32);

			default:
				// it's not related on rendering things i guess
				#if (!macro && lime_cffi)
				#if !cs
