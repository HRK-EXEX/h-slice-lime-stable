...
class ImageDataUtil
{
	...
	static var getPixelbytes:Bytes;
	public static function getPixels(image:Image, rect:Rectangle, format:PixelFormat):Bytes
	{
		...
		if (getPixelbytes == null || getPixelbytes.length != length * 4)
		{
			getPixelbytes = Bytes.alloc(length * 4);
		}
		// var bytes = Bytes.alloc(length * 4);

		#if (lime_cffi && !disable_cffi && !macro)
		if (CFFI.enabled) NativeCFFI.lime_image_data_util_get_pixels(image, rect, format, getPixelbytes);
		else
		#end
		{
			var data = image.buffer.data;
			...
			for (y in 0...dataView.height)
			{
				...
				for (x in 0...dataView.width)
				{
					...
					getPixelbytes.set(destPosition++, pixel.r);
					getPixelbytes.set(destPosition++, pixel.g);
					getPixelbytes.set(destPosition++, pixel.b);
					getPixelbytes.set(destPosition++, pixel.a);
